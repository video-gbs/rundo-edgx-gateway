...
import com.runjian.gb28181.transmit.event.response.ISIPResponseProcessor;
import com.runjian.gb28181.transmit.event.timeout.ITimeoutProcessor;
...
public class SIPProcessorObserver implements ISIPProcessorObserver {
    ...
    public void processResponse(ResponseEvent responseEvent) {
        ...
        if (((status >= Response.OK) && (status < Response.MULTIPLE_CHOICES)) || status == Response.UNAUTHORIZED) {
            ...
            logger.warn(LogTemplate.PROCESS_LOG_TEMPLATE, "SIP信令处理类观察者", String.format("接收到失败的response响应！status:%s,message:%s", status, response));
    ...
    public void processTimeout(TimeoutEvent timeoutEvent) {
        ...
        if (clientTransaction != null) {
            logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "SIP信令处理类观察者", "[发送错误订阅] clientTransaction != null");
            Request request = clientTransaction.getRequest();
            if (request != null) {
                logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "SIP信令处理类观察者", "[发送错误订阅] request != null");
                CallIdHeader callIdHeader = (CallIdHeader) request.getHeader(CallIdHeader.NAME);
                if (callIdHeader != null) {
                    logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "SIP信令处理类观察者", "[发送错误订阅]");
                    SipSubscribe.Event subscribe = sipSubscribe.getErrorSubscribe(callIdHeader.getCallId());
