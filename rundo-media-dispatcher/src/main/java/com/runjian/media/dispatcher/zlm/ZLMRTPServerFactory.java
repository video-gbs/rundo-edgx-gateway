...
import com.runjian.media.dispatcher.zlm.dto.SendRtpItem;
...
public class ZLMRTPServerFactory {
    ...
    public int getFreePort(MediaServerItem mediaServerItem, int startPort, int endPort, List<Integer> usedFreelist) {
        ...
        if (!checkPort) {
            logger.warn(LogTemplate.PROCESS_LOG_TEMPLATE, "ZLM RTP Server", String.format("未找到节点%s上范围[%s-%s]的空闲端口",mediaServerItem.getId(), startPort, endPort));
            return -1;
        }
...
//        param.put("port", 0);
        ...
        if (openRtpServerResultJson != null) {
            ...
            //  检查ZLM状态
            logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "ZLM RTP Server", String.format("创建RTP Server 失败 %s: 请检查ZLM服务",param.get("port")) ,null);
        }
        return result;
    }
    ...
    public int createRTPServer(MediaServerItem mediaServerItem, String streamId, int ssrc, Integer port) {
                ...
                if (result == 0) {
                    ...
                    if (jsonObject != null ) {
                        if (jsonObject.getInteger("code") == 0) {
                            ...
                            logger.warn(LogTemplate.PROCESS_LOG_TEMPLATE, "ZLM RTP Server", "[开启rtpServer], 重启RtpServer错误");
                        }
                    }
                }
                return result;
        ...
        param.put("ssrc", ssrc);
        JSONObject openRtpServerResultJson = zlmresTfulUtils.openRtpServer(mediaServerItem, param);
        logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "ZLM RTP Server", JSONObject.toJSONString(openRtpServerResultJson));
        if (openRtpServerResultJson != null) {
            if (openRtpServerResultJson.getInteger("code") == 0) {
                result= openRtpServerResultJson.getInteger("port");
            }else {
                logger.info(LogTemplate.PROCESS_LOG_MSG_TEMPLATE, "ZLM RTP Server", "创建RTP Server 失败", openRtpServerResultJson.getString("msg"));
            }
        }else {
            //  检查ZLM状态
            logger.error(LogTemplate.ERROR_LOG_MSG_TEMPLATE, "ZLM RTP Server", "创建RTP Server 失败,请检查ZLM服务", param.get("port"), null);
        }
        return result;
    }
        ...
        if (serverItem !=null){
            ...
            if (jsonObject != null ) {
                if (jsonObject.getInteger("code") == 0) {
                    ...
                    logger.error(LogTemplate.ERROR_LOG_MSG_TEMPLATE, "ZLM RTP Server", "关闭RTP Server 失败", jsonObject.getString("msg"), null);
                }
            }else {
                //  检查ZLM状态
                logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "ZLM RTP Server", "关闭RTP Server 失败: 请检查ZLM服务", null);
            }
        }
        return result;
    }
...
//    }
    ...
    public SendRtpItem createSendRtpItem(MediaServerItem serverItem, String ip, int port, String ssrc, String platformId, String deviceId, String channelId, boolean tcp){
        ...
        if (localPort == -1) {
            logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "ZLM RTP Server", "没有可用的端口", null);
    ...
    public Boolean stopSendRtpStream(MediaServerItem mediaServerItem, Map<String, Object>param) {
        ...
        if (jsonObject == null) {
            logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "ZLM RTP Server", "[停止RTP推流] 失败: 请检查ZLM服务", null);
        } else if (jsonObject.getInteger("code") == 0) {
            result= true;
            logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "ZLM RTP Server", "[停止RTP推流] 成功");
        } else {
            logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "ZLM RTP Server", String.format("[停止RTP推流] 失败: %s, 参数：%s->\r\n%s",jsonObject.getString("msg"),JSONObject.toJSON(param), jsonObject), null);
        }
        return result;
    }
