    ...
    public void startCron(String key, Runnable task, int cycleForCatalog) {
        ...
        if (future != null) {
            if (future.isCancelled()) {
                logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "动态定时任务", String.format("任务【%s】已存在但是关闭状态！！！", key));
            } else {
                logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "动态定时任务", String.format("任务【%s】已存在且已启动！！！", key));
                return;
            }
        }
