...
//			logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "Register响应处理器", String.format("[国标级联]未找到callId： %s 的注册/注销平台id", callId));
//			return;
//		}
//
//		ParentPlatformCatch parentPlatformCatch = redisCatchStorage.queryPlatformCatchInfo(platformRegisterInfo.getPlatformId());
//		if (parentPlatformCatch == null) {
//			logger.warn(LogTemplate.PROCESS_LOG_TEMPLATE, "Register响应处理器", String.format("[国标级联]收到注册/注销%S请求，平台：%s，但是平台缓存信息未查询到!!!", response.getStatusCode(),platformRegisterInfo.getPlatformId()));
//			return;
//		}
//
//		String action = platformRegisterInfo.isRegister() ? "注册" : "注销";
//		logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "Register响应处理器", String.format("[国标级联]%s %S响应,%s ", action, response.getStatusCode(), platformRegisterInfo.getPlatformId() ));
//		ParentPlatform parentPlatform = parentPlatformCatch.getParentPlatform();
//		if (parentPlatform == null) {
//			logger.warn(LogTemplate.PROCESS_LOG_TEMPLATE, "Register响应处理器", String.format("[国标级联]收到 %s %s的%S请求, 但是平台信息未查询到!!!", platformRegisterInfo.getPlatformId(), action, response.getStatusCode()));
//			return;
//		}
//
...
//				logger.error(LogTemplate.ERROR_LOG_TEMPLATE, "Register响应处理器", "[命令发送失败] 国标级联 再次注册", e);
//			}
