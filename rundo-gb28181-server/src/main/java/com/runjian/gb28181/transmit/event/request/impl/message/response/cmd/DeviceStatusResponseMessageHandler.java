...
public class DeviceStatusResponseMessageHandler extends SIPRequestProcessorParent implements InitializingBean, IMessageHandler {
    ...
    public void handForDevice(RequestEvent evt, Device device, Element element) {
        logger.info(LogTemplate.PROCESS_LOG_TEMPLATE, "DeviceStatus应答消息处理", "接收到DeviceStatus应答消息");
        // 检查设备是否存在， 不存在则不回复
        if (device == null) {
            return;
        }
        ...
        try {
            ...
            logger.error(LogTemplate.ERROR_LOG_MSG_TEMPLATE, "DeviceStatus应答消息处理", "设备状态应答回复200OK 命令发送失败", e.getMessage(), e);
